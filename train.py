...
class SSCD(pl.LightningModule):
    ...
    def __init__(self, args, train_steps: int):
        ...
        if self.kd:
            ...
            state_dict['embeddings.0.weight'] = state_dict.pop('backbone.fc.weight')
            state_dict['embeddings.0.bias'] = state_dict.pop('backbone.fc.bias')
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.kd:
            ...
            infonce_loss = self.loss(emb[-1][:,:self.direct], instance_ids)
            kd_loss = self.kd_loss(emb[0], emb_t[0])
            loss = infonce_loss + kd_loss
        else:
            emb = self(input)
            loss = self.loss(emb[-1][:,:self.direct], instance_ids)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.kd:
            batch["embeddings"] = self(input)[0][0]
        else:
